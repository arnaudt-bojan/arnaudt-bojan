...
-   **Inventory Management System**: Production-ready transaction-based stock reservation with atomic operations, PostgreSQL row-level locking, and automatic stock synchronization. Features unified variant stock management across all operations (decrement/increment/commit) with multi-format variantId support ("size-color" full format and "size" fallback), automatic product.stock auto-sync (recalculated from sum of all variant stocks on every change), comprehensive logging for stock reservations/commits/decrements, and frontend cache invalidation for real-time stock updates. Handles both nested color→sizes variant structure `[{colorName, sizes: [{size, stock}]}]` and simple size-only structure `[{size, stock}]`. Stock restoration on order cancellation/refund mirrors the decrement logic to ensure accurate inventory recovery.
-   **Cart Reservation System (Architecture 3)**: Industry best-practice stock reservation lifecycle managed via dedicated `CartReservationService`. Implements "soft hold" pattern: items in cart have NO reservation (UI only), reservations created only when checkout initiates (30-min expiry), automatic release via background cleanup job. Features: reservation extension for active checkout sessions, manual release on cancel/abandon, atomic commit on payment success. API endpoints: `POST /api/cart/reservations/extend` (extend time), `DELETE /api/cart/reservations/:sessionId` (release), `GET /api/cart/reservations/:sessionId` (status). Race conditions prevented through database transactions with row-level locking. Reservation expiration: 30 minutes (industry standard), cleanup runs every 5 minutes. Service handles: createCheckoutReservations, extendReservation, releaseSessionReservations, commitReservationsToOrder, hasActiveReservations.
