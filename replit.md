...
- **Image Upload System** - Implemented complete file upload infrastructure using Replit object storage. Sellers can now upload product images directly (in addition to URL input). Features: ObjectUploader component with Uppy integration, presigned URL workflow for secure uploads, automatic ACL policy management for public image access, dual input method (URL paste OR file upload) for maximum flexibility. Each image slot includes an Upload button alongside URL input field. Backend endpoints: POST /api/objects/upload (presigned URLs), PUT /api/product-images (ACL policies), GET /objects/:objectPath (image serving).
- **Multi-Provider Payment System** - Redesigned payment infrastructure to support multiple payment providers (Stripe, PayPal). Removed pre-selected default provider - sellers now explicitly choose which provider(s) to connect. Updated settings UI with provider comparison cards showing benefits (currencies, features, platform fee). All providers support: direct payments to seller (no fund custody), 1.5% platform fee, and seller's name on buyer statements.
- **Stripe Connection Fix** - Fixed critical bug where Stripe connection failed with "Unexpected token '<'" JSON parse error. Updated frontend to properly call Express account creation (POST /api/stripe/create-express-account) followed by account link generation (POST /api/stripe/account-link) for onboarding flow.
...
- **PayPal Marketplace Integration** - Implemented PayPal Commerce Platform partner integration with merchant referral flow and onboarding status tracking (foundation laid for future PayPal credentials). UI shows PayPal as "Coming Soon" with feature comparison.
