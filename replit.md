...
-   **Pricing Display Architecture (Architecture 3 Compliance):** Strict separation between seller-side (NO conversion) and buyer-side (WITH conversion) pricing displays. Seller-side components (product cards, product lists, seller dashboard) use `formatPrice()` from `@/lib/currency-utils` with product's listing currency (`(product as any).currency` enriched by ProductService) - NO conversion, just formatting. Buyer-side components (cart, checkout) use `CurrencyContext.formatPrice()` for currency conversion based on user preference. Products are enriched with seller's `listingCurrency` as a `currency` field via `ProductService.getProductWithCurrency()`. This prevents incorrect price displays where €100 EUR products appear as $100 USD converted to €85.45.
