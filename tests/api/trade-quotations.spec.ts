...
describe("Trade Quotation System @api @integration @trade", () => {
  ...
  it("should calculate deposit and balance amounts correctly", async () => {
    ...
    // Deposit (30%): $3,000
    // Balance: $7,000
    expect(parseFloat(quotation.total)).toBe(10000.0);
    expect(parseFloat(quotation.deposit_amount || "0")).toBe(3000.0);
    expect(parseFloat(quotation.balance_amount || "0")).toBe(7000.0);
  });
  ...
  it("should handle quotation expiration", async () => {
    ...
    const expiredQuotation = await fixtures.createTradeQuotation(
      sellerId,
      "buyer@example.com",
      {
        status: "sent",
