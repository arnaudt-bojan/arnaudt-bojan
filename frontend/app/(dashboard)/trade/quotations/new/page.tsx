...
import { SUPPORTED_CURRENCIES, DEFAULT_CURRENCY } from '@/lib/shared';
...
export default function QuotationBuilder() {
  ...
  const handleSubmit = async () => {
    if (!buyerEmail || items.some(item => !item.description || item.quantity <= 0)) {
      setSnackbar({
        open: true,
    ...
    await createQuotation({ variables: { input } });
  };
  ...
  return (
    <DashboardLayout title="New Quotation">
      <Container maxWidth="xl" sx={{ py: 4 }} data-testid="page-create-quotation">
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box>
            ...
            <Typography variant="body1" color="text.secondary">
              Build a professional quotation for your trade buyer
            </Typography>
          </Box>
          <Button
            variant="outlined"
        ...
        <Grid container spacing={3}>
          <Grid size={{ xs: 12, md: 8 }}>
            <Card sx={{ mb: 3 }}>
              <CardHeader title={<Typography variant="h6">Buyer Information</Typography>} />
              <CardContent>
