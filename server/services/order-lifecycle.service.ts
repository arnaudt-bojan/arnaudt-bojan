...
export class OrderLifecycleService {
  ...
  async updateOrderStatus(orderId: string, status: string, userId: string): Promise<UpdateOrderStatusResult> {
    try {
      ...
      // Cascade status to all order items
      // When order status changes, update all item statuses to match
      const orderItems = await this.storage.getOrderItems(orderId);
      const itemStatusMapping: Record<string, string> = {
        'pending': 'pending',
      ...
      const newItemStatus = itemStatusMapping[status] || status;
      
      // Update all order items to the new status
      for (const item of orderItems) {
        // Only update if the item isn't already in a terminal state (refunded, cancelled, returned)
        if (item.itemStatus !== 'refunded' && item.itemStatus !== 'cancelled' && item.itemStatus !== 'returned') {
          await this.storage.updateOrderItemStatus(item.id, newItemStatus);
        }
      }

      logger.info(`[Order Status] Updated order ${orderId} and ${orderItems.length} items to status: ${status}`);
