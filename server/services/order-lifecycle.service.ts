...
export class OrderLifecycleService {
  ...
  async updateOrderStatus(orderId: string, status: string, userId: string): Promise<UpdateOrderStatusResult> {
    try {
      ...
      // ALSO update all order_items status to match order status
      // This ensures UI and item-level tracking are in sync
      const orderItems = await this.storage.getOrderItems(orderId);
      for (const item of orderItems) {
        // Map order status to appropriate item status
        let itemStatus = status;
        if (status === 'delivered' || status === 'shipped') {
          itemStatus = status; // delivered or shipped
        } else if (status === 'cancelled') {
          itemStatus = 'cancelled';
        } else if (status === 'processing') {
          itemStatus = 'processing';
        } else {
          itemStatus = 'pending'; // default fallback
        }
        
        await this.storage.updateOrderItemStatus(item.id, itemStatus);
      }
