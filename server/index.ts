...
app.get("/api/metrics/socketio", async (_req, res) => {
  try {
    ...
    const healthStatus = {
      isHealthy:
        metrics.activeConnections >= 0 && metrics.connectionErrors < 100,
      errorRate:
        metrics.totalConnections > 0
          ? (
              (metrics.connectionErrors / metrics.totalConnections) *
              100
            ).toFixed(2) + "%"
          : "0%",
      authFailureRate:
        metrics.totalConnections > 0
          ? (
              (metrics.authenticationFailures / metrics.totalConnections) *
              100
            ).toFixed(2) + "%"
          : "0%",
    };
          ...
          tiers: {
            global: {
              ...
              windowSeconds: parseInt(
                process.env.RATE_LIMIT_GLOBAL_WINDOW || "60",
                10,
              ),
            },
            auth: {
              ...
              windowSeconds: parseInt(
                process.env.RATE_LIMIT_AUTH_WINDOW || "900",
                10,
              ),
            },
            user: {
              maxRequests: parseInt(
                process.env.RATE_LIMIT_USER_MAX || "100",
                10,
              ),
              windowSeconds: parseInt(
                process.env.RATE_LIMIT_USER_WINDOW || "60",
                10,
              ),
            },
            endpoint: {
              maxRequests: parseInt(
                process.env.RATE_LIMIT_ENDPOINT_MAX || "1000",
                10,
              ),
              windowSeconds: parseInt(
                process.env.RATE_LIMIT_ENDPOINT_WINDOW || "60",
                10,
              ),
            },
          },
...
function generateCacheRecommendations(metrics: any): string[] {
  ...
  if (metrics.hitRate < 50) {
    recommendations.push(
      "Low hit rate detected. Consider increasing CACHE_TTL values or reviewing cache key patterns.",
    );
  }

  if (metrics.currentSize > metrics.maxSize * 0.9) {
    recommendations.push(
      "Cache near capacity. Consider increasing CACHE_MAX_SIZE.",
    );
  }

  if (metrics.averageLatencyMs > 10) {
    recommendations.push(
      "High cache latency detected. Consider using Redis for better performance.",
    );
  }

  if (metrics.backend.includes("fallback")) {
    recommendations.push(
      "Running on fallback cache. Check Redis connection if Redis is configured.",
    );
  }
...
function generateRateLimiterRecommendations(metrics: any): string[] {
  ...
  if (metrics.blockRate > 20) {
    recommendations.push(
      "High block rate detected. Consider reviewing rate limit thresholds or identifying potential attacks.",
    );
  }

  if (metrics.violationsByTier.auth > 100) {
    recommendations.push(
      "High authentication violations. Possible brute force attack - review security logs.",
    );
  }

  if (metrics.violationsByTier.global > 1000) {
    recommendations.push(
      "High global violations. Consider enabling IP allowlist or implementing DDoS protection.",
    );
  }

  if (metrics.backend.includes("fallback")) {
    recommendations.push(
      "Running on fallback rate limiter. Check Redis connection if Redis is configured.",
    );
  }

  if (metrics.backend === "memory" && metrics.totalRequests > 100000) {
    recommendations.push(
      "High traffic detected with in-memory backend. Consider switching to Redis for better scalability.",
    );
  }
...
(async () => {
  ...
  log(
    "[Server] Production-grade rate limiting enabled (token bucket algorithm)",
  );
  ...
  server.listen(
    ...
    async () => {
      ...
      balanceReminderJob = new WholesaleBalanceReminderJob(
        storage,
        notificationService,
      );
      ...
      if (
        ...
        try {
          ...
          log(
            "[Server] Failed to start Meta Ads jobs: " +
              (error instanceof Error ? error.message : "Unknown error"),
          );
        }
      } else {
        log(
          "[Server] Meta Ads background jobs skipped (META_APP_ID, META_APP_SECRET, or STRIPE_SECRET_KEY not configured)",
        );
      }
      ...
      if (process.env.SHIPPO_API_KEY) {
        const shippoLabelService = new ShippoLabelService(
          storage,
          notificationService,
        );
        ...
        (async () => {
          try {
            ...
            log(
              "[Shippo Refund Poll] Initial poll failed: " +
                (error instanceof Error ? error.message : "Unknown error"),
            );
          }
        })();
        ...
        shippoRefundPollInterval = setInterval(
          async () => {
            try {
              await shippoLabelService.pollPendingRefunds();
              ...
              log(
                "[Shippo Refund Poll] Poll cycle failed: " +
                  (error instanceof Error ? error.message : "Unknown error"),
              );
            }
          },
        ...
        log(
          "[Server] Shippo refund polling skipped (SHIPPO_API_KEY not configured)",
        );
      }
